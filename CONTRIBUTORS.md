...
- ⚖️ **Respetuoso con NASA** - Intentando alinearse con sus directrices públicas y corrigiendo la documentación cuando haga falta
...
- **Contenido NASA:** Respetar la política documental del proyecto y revisar condiciones específicas de cada asset cuando corresponda
...
```powershell
# Suite Python
python -m pip install --upgrade -r scripts\requirements-dev.txt
python -m ruff check scripts tests\python
python -m pytest -q tests\python

# Suite PowerShell (Pester)
Invoke-Pester tests\powershell

# Simulación segura del instalador
.\install.ps1 -Theme dark -WhatIf
```

La automatización de `.forgejo/workflows/ci.yml` repite estas comprobaciones y
valida el checksum del ejecutable publicado.
